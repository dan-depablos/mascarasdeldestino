...
@onready var cactobrotwo: CharacterBody2D = $"."
@onready var cactocollisionshape: CollisionShape2D = $cactocollisionshape
...
func _physics_process(delta: float) -> void:
	if Globalscript.cactobrotwo == false:
		cactobrotwo.visible = false
		cactocollisionshape.disabled = true
		hitarea.monitoring = false
		
	else:
		cactobrotwo.visible = true
		cactocollisionshape.disabled = false
